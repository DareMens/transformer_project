...
from modelling.model import TransformerDecoderLayer
...
TEST_DATA = [
    (
        TransformerDecoderLayer(
            d_model=INPUT.size(-1), num_heads=2, feature_dim=6, dropout=0.0
        ),
        INPUT,
        ENCODER,
        ATTENTION_MASK,
...
STATE_DICT = {
    ...
    "cross_attention.query_transform.weight": torch.tensor(
        [
    ...
    "cross_attention.key_transform.weight": torch.tensor(
        [
    ...
    "cross_attention.value_transform.weight": torch.tensor(
        [
    ...
    "cross_attention.output_transform.weight": torch.tensor(
        [
    ...
    "feature_transformation.feed_forward.0.weight": torch.tensor(
        [
    ...
    "feature_transformation.feed_forward.0.bias": torch.tensor(
        [-0.6578, 0.0737, -1.1023, 1.0154, -0.0636, 0.6766]
    ),
    "feature_transformation.feed_forward.2.weight": torch.tensor(
        [
    ...
    "feature_transformation.feed_forward.2.bias": torch.tensor(
...
def test_layer(layer, input, encoder, encoder_attention_mask, attention_mask, expected):
    ...
    assert torch.allclose(actual.detach(), expected, rtol=1e-04, atol=1e-06)