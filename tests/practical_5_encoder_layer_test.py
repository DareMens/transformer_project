...
TEST_DATA = [
    (
        TransformerEncoderLayer(d_model=INPUT.size(-1), num_heads=2, feature_dim=6, dropout=0.0),
        INPUT,
        ATTENTION_MASK,
        torch.tensor([
...
STATE_DICT = {
    ...
    'feed_forward.feed_forward.0.weight': torch.tensor([[-0.6284, -0.6438,  1.5350,  1.1490],
    ...
    'feed_forward.feed_forward.0.bias': torch.tensor([-0.6578,  0.0737, -1.1023,  1.0154, -0.0636,  0.6766]),
    'feed_forward.feed_forward.2.weight': torch.tensor([[-0.1321, -0.8338,  1.9037,  1.3632, -0.5201, -0.1429],
    ...
    'feed_forward.feed_forward.2.bias': torch.tensor([0.2078,  0.2490,  0.0818, -0.0595]),
    'layer_norm1.weight': torch.tensor([1., 1., 1., 1.]),
    'layer_norm1.bias': torch.tensor([0., 0., 0., 0.]),
    'layer_norm2.weight': torch.tensor([1.1, 1.1, 1.1, 1.1]),
    'layer_norm2.bias': torch.tensor([0., 0., 0., 0.])
}
